...
- MRZ chi dung de khop dung nguoi; ngay cap phai doc tu vung mat sau, gan nhan ngay cap hop le. CCCD mau cu chap nhan dong `Ngay, thang, nam / Date, month, year`; Can cuoc mau moi chap nhan dong `Ngay, thang, nam cap / Date of issue`.
- Ngay cap chi duoc lay tren cung dong voi nhan ngay cap hoac toi da 2 dong ke tiep. Chap nhan cac dinh dang `dd/MM/yyyy`, `dd-MM-yyyy`, `dd.MM.yyyy` va `ddMMyyyy` khi OCR lam mat dau `/`.
- Khong duoc tu suy luan ngay cap tu OCR hong/bien dang nhu `Oro712021`, `Or0712021` thanh mot ngay cu the. Neu OCR full image khong doc ro, he thong phai dung crop chuyen biet: CCCD cu crop hep sau anchor `year/yea`; Can cuoc moi crop vung ben duoi `Date of issue`. Khong dung OpenAI Vision/toan anh lam nguon quyet dinh doc lap cho ngay cap; neu crop khong co dung mot ngay hop le thi de trong/canh bao sua tay.
- Neu khong co nhan ngay cap hop le, khong khop dung mat sau theo so CCCD/Can cuoc, co nhieu ngay ung vien khac nhau, hoac ngay khong hop le thi de trong/canh bao sua tay; khong lay ngay gan nhat.
...
- Ten loai GCN tren bia dat co 03 loai chuan can nhan dien va ghi dung vao review/hop dong: `Giay chung nhan quyen su dung dat, quyen so huu tai san gan lien voi dat`; `Giay chung nhan quyen su dung dat, quyen so huu nha o va tai san khac gan lien voi dat`; `Giay chung nhan quyen su dung dat`. OCR/bia dat thuoc loai nao thi cau mo ta tai san trong hop dong phai ghi dung loai do tai doan `theo ... so ...`; khong hard-code mot loai duy nhat. Loi OCR ro rang nhu `hwuux`/`hwux`/`huux` trong cum `quyen so huu` thi sua thanh `huu/hữu`.
...
- Truong `Thoi han su dung` phai lay theo dung muc `e)` tren bia dat, khong duoc ep co dinh thanh `lau dai` vi co the co nhieu cach ghi khac nhu `Den ngay ...`, `Su dung den ...`. Chi duoc sua loi OCR chinh ta ro rang trong chinh truong nay, vi du OCR doc nham `dai` thanh nghia `dai/castle` thi sua lai thanh `dai/long-term` theo hoa-thuong cua bia; cac cau khac giu nguyen.
