...
- Build review layout from a semantic document model, not from hard-coded item letters, fixed item counts, or one sample's field order. The complete certificate title selects the certificate generation; detected section headings and field-label meanings select the fields and their displayed positions.
- Maintain one label ontology for extraction and review presentation. Each recognized label has a semantic field key, one or more standard printed-wording variants, and a certificate scope. OCR spelling repair may map a noisy label to that ontology only when the complete phrase is sufficiently similar; the item marker alone never determines field meaning.
- Parse each normalized page/region independently and merge semantic items afterward. Never concatenate pages in an arbitrary order and then stop parsing the whole certificate when a later-page heading such as `III` or `IV` appears before a separately stored land block.
- Preserve the detected OCR order in the semantic model for traceability. The review presentation may order recognized fields by their semantic/template positions, but never by assumed markers such as `a-g`; markers are presentation metadata and boundaries only. Unknown/unmapped fields must keep their detected order so they can be reviewed instead of being swallowed into a neighboring value.
- When OCR loses or corrupts a marker but the label phrase is recognizable, keep the semantic field and display the repaired standard label wording for that detected variant. Examples include correcting encoding noise such as `mÂ²` to `m²` and obvious label OCR such as `CCCP sa` to `CCCD số` only when surrounding focused evidence supports the intended fixed label.
